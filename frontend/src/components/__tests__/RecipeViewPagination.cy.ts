...
describe('Render', () => {
  it('Render properly', () => {
    ...
    for (let id = 1; id <= 5; ++id) {
      cy.get(`[data-test=recipe-view-pagination-${id}`).should('be.visible');
    }

    cy.get('[data-test=recipe-view-pagination-pagination]')
      .should('be.visible')

      .get('[data-test=recipe-view-pagination-error-dialog]')
      .should('not.exist')
      .get('[data-test=recipe-view-pagination-loading-dialog]')
      .should('not.exist')

      // < 1 2 3 4 5 ... 7 > (total 9 lis)
      .get('[data-test=recipe-view-pagination-pagination] li')
...
describe('Pagination', () => {
  it('Move to second page', () => {
    ...
    cy.mount(() => h(RecipeViewPagination))
      .get('[data-test=recipe-view-pagination-pagination] li button span')
      .each(($elem) => {
        if ($elem.text() === '2') {
