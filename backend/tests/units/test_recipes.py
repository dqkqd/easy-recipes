...
def test_200_create_basic(client: FlaskClient) -> None:
    recipe_create = mock_data.MockRecipe.random_valid_recipe()
...
def test_422_create_invalid_name(client: FlaskClient, name: str | None) -> None:
    recipe_data = mock_data.MockRecipe.random_valid_recipe_data()
...
def test_200_get_recipe_basic(client: FlaskClient) -> None:
    ...
    data = json.loads(response.data)
    recipe_public = RecipePublic(**data)

    assert response.status_code == 200
    assert recipe_create.model_dump(
        exclude={"image_uri"},
    ) == recipe_public.model_dump(exclude={"id", "image_uri"})

    assert recipe_create.image_uri != recipe_public.image_uri
    assert compare_image_data_from_uri(
        recipe_create.image_uri,
        recipe_public.image_uri,
    )
...
def test_200_get_recipe_after_many_create(client: FlaskClient) -> None:
    ...
    for _ in range(num_datas):
        client.post(
            "/recipes/",
            json=mock_data.MockRecipe.random_valid_recipe_data(),
        )
...
def test_200_get_all(client: FlaskClient) -> None:
    ...
    for _ in range(num_datas):
        data = mock_data.MockRecipe.random_valid_recipe_data()
